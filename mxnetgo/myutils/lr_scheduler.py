...
class StepScheduler(LRScheduler):
    ...
    def __init__(self, epoch_update_nums,steplist, factor=1):
        ...
        self.base_lr = self.previous_lr = steplist[0][1]
    ...
    def __call__(self, num_update):
        ...
        for (e,v) in self.steplist:
            if num_update < self.epoch_update_nums*e:
                if self.previous_lr != v:
                    logger.info("lr changed from: {} to {}".format(self.previous_lr, v))
                self.previous_lr = self.base_lr = v
                return self.base_lr
