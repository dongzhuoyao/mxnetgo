...
base_dir, LIST_DIR = "/data1/dataset/pascalvoc2012/VOC2012trainval/VOCdevkit/VOC2012", "../data/pascalvoc12"
...
def test_deeplab(ctx):
    #logger.auto_set_dir()
    test_data = get_data("val", base_dir, LIST_DIR, len(ctx))
...
def train_net(args, ctx):
    ...
    train_data = get_data("train_aug", base_dir, LIST_DIR, len(ctx))
    test_data = get_data("val", base_dir, LIST_DIR, len(ctx))
...
def view_data(ctx):
        ds = get_data("train_aug", base_dir, LIST_DIR, ctx)
        ds.reset_state()
