...
class resnet101_deeplab_new(Symbol):
    ...
    def get_symbol(self, num_class, is_train, units=[3, 4, 23, 3], num_stage=4, filter_list=[64, 256, 512, 1024, 2048],b_lr_mult=2.0,w_lr_mult=1.0, bottle_neck=True, bn_mom=0.9, workspace=512, memonger=False):
        ...
        def aspp(input_sym):
            ...
            sum_ = sum_+ mx.symbol.Convolution(data=relu_fc6, kernel=(1, 1), pad=(0, 0), num_filter=num_class, name="score",
                                          bias=score_bias, weight=score_weight, workspace=workspace)
            for i in range(3):
                score_bias_i = mx.symbol.Variable('score_bias{}'.format(i+1), lr_mult=b_lr_mult)
                score_weight_i = mx.symbol.Variable('score_weight{}'.format(i+1), lr_mult=w_lr_mult)
                sum_ = sum_ + mx.symbol.Convolution(data=relu_fc6, kernel=(1, 1), pad=(0, 0),dilate=(6*(i+1),6*(i+1)), num_filter=num_class, name="score",
                                          bias=score_bias_i, weight=score_weight_i, workspace=workspace)

            """Image Pooling
            See ParseNet: Looking Wider to See Better
            """
            tmp = mx.symbol.mean(relu_fc6,axis=(2,3),keepdims=True)
    ...
    def init_weights(self, arg_params, aux_params):
        ...
        arg_params['score_weight1'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['score_weight1'])
        arg_params['score_bias1'] = mx.nd.zeros(shape=self.arg_shape_dict['score_bias1'])
        arg_params['score_weight2'] = mx.random.normal(0, 0.01, shape=self.arg_shape_dict['score_weight2'])
        arg_params['score_bias2'] = mx.nd.zeros(shape=self.arg_shape_dict['score_bias2'])
