...
from mxnetgo.tensorpack.dataset.pascalvoc12 import PascalVOC12
from tensorpack.dataflow.imgaug.misc import  Flip,RandomResize
...
def test_deeplab(ctx):
    ...
    for data, label in tqdm(test_data.get_data()):
        ...
        if args.vis:
            cv2.imwrite(os.path.join(vis_dir,"{}.jpg".format(nbatch)),np.concatenate((data[0][0], visualize_label(output_all)), axis=1))
