...
from mxnetgo.myutils.seg_utils import RandomCropWithPadding
from tensorpack.dataflow.imgaug.misc import RandomResize
...
def get_data(name, data_dir, meta_dir, gpu_nums):
    ...
    if isTrain:
        shape_aug = [
                RandomResize(xrange=(0.7, 1.5), yrange=(0.7, 1.5),
                         aspect_ratio_thres=0.15, interp=cv2.INTER_NEAREST),
...
def proceed_validation(ctx):
    ...
    if args.vis:
        ...
        vis_dir = os.path.join("validation_result_camvid_edgeremoved","vis")
        mkdir_p(vis_dir)
    ...
    for data, label in tqdm(test_data.get_data()):
        ...
        if args.vis:
            ...
            cv2.imwrite(os.path.join(vis_dir,"{}.jpg".format(nbatch)),np.concatenate((origin_img, visualize_label(label), np.dstack((label,label,label)), visualize_label(output_all),), axis=1))
